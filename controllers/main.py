...
class WebsiteSaleCircularShipping(WebsiteSale):
    ...
    def _cs_get_payment_values(self, order):
        ...
        popup_text_raw = getattr(w, f'cs_popup_text_{lang_key}', '') or ''
        cs_popup_text = Markup(popup_text_raw) if popup_text_raw else Markup('')
        cs_explainer_reusable   = getattr(w, f'cs_explainer_reusable_{lang_key}', '') or ''
        cs_explainer_single_use = getattr(w, f'cs_explainer_single_use_{lang_key}', '') or ''
        ...
        _full_config = {
            ...
            'default_selection': w.cs_default_selection or 'single_use',
        }
        current_choice = ''
        if order:
            current_choice = (
        ...
        cs_box_image_url = w.cs_box_image_url or ''
        dark_mode = bool(w.cs_dark_mode)
    ...
    def set_packaging_choice(self, choice=None, **kwargs):
        if not request.website.cs_enabled:
    ...
    def _get_ab_variant(self):
        w = request.website
        if not w.cs_ab_test_enabled:
            return ''
        if 'cs_ab_variant' not in request.session:
            ratio   = float(w.cs_ab_split_ratio or 0.5)
